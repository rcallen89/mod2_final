...
RSpec.describe "Merchant Items Index Page" do
  describe "When I visit the merchant items page" do
    ...
    it 'when a merchant employee visits their item page they can active items' do
      ...
      expect(page).to have_content("#{@chain.name} now available for sale")
    ...
    it 'when a merchant employee visits their item page they can dective items' do
      user = create(:user, role: 1)
      @meg.users << user
      allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(user)
